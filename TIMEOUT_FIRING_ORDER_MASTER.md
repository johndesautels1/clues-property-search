# CLUES Property Search - Timeouts & Firing Order Master Reference

## Quick Reference Table

| Tier | Source | Timeout | Firing Order | File(s) |
|------|--------|---------|--------------|---------|
| 1 | Stellar MLS (Bridge API) | 15s | 1st | search.ts, search-by-mls.ts |
| 2 | Google Geocoding | 60s | 2nd | search.ts |
| 2 | Google Places | 60s | 2nd | search.ts |
| 2 | Google Distance Matrix | 60s | 2nd | search.ts |
| 3 | SchoolDigger | 60s | 3rd (parallel) | search.ts |
| 3 | FBI Crime | 60s | 3rd (parallel) | search.ts |
| 3 | WalkScore | 60s | 3rd (parallel) | search.ts |
| 3 | FEMA NFHL | 60s | 3rd (parallel) | search.ts |
| 3 | AirNow | 60s | 3rd (parallel) | search.ts |
| 3 | HowLoud | 60s | 3rd (parallel) | search.ts |
| 3 | OpenWeatherMap | 60s | 3rd (parallel) | search.ts |
| 3 | U.S. Census | 60s | 3rd (parallel) | search.ts |
| 3 | **Tavily Web Search** | **15s** | 3rd (after Free APIs) | search.ts, tavily-search.ts |
| 4 | Perplexity | 45s | 4th - LLM #1 | search.ts, retry-llm.ts, perplexity-prompts.ts |
| 4 | Gemini | 180s | 5th - LLM #2 | search.ts, retry-llm.ts, gemini-prompts.ts |
| 4 | GPT-4o | 180s | 6th - LLM #3 | search.ts, retry-llm.ts |
| 4 | Claude Sonnet | 180s | 7th - LLM #4 | search.ts, retry-llm.ts |
| 4 | Grok | 180s | 8th - LLM #5 | search.ts, retry-llm.ts |
| 5 | Claude Opus | 180s | 9th - LLM #6 (LAST) | search.ts, retry-llm.ts |

## Vercel Serverless Config

| File | maxDuration |
|------|-------------|
| search.ts | 300s (5 min) |
| search-by-mls.ts | 300s (5 min) |
| search-stream.ts | 300s (5 min) |
| retry-llm.ts | 300s (5 min) |

## Timeout Constants by File

### search.ts (Main Orchestrator)
```typescript
const STELLAR_MLS_TIMEOUT = 15000;   // 15s - Tier 1
const FREE_API_TIMEOUT = 60000;       // 60s - Tier 2 & 3 Free APIs
const TAVILY_TIMEOUT = 15000;         // 15s - Tier 3 Tavily
const PERPLEXITY_TIMEOUT = 45000;     // 45s - Tier 4 LLM #1
const LLM_TIMEOUT = 180000;           // 180s - Tier 4-5 LLMs #2-6
```

### retry-llm.ts (Single LLM Retry)
```typescript
const LLM_TIMEOUT = 180000;           // 180s - GPT, Gemini, Sonnet, Grok, Opus
const PERPLEXITY_TIMEOUT = 45000;     // 45s - Perplexity
```

### search-by-mls.ts (MLS-First Flow)
```typescript
const BRIDGE_MLS_TIMEOUT = 15000;     // 15s - Bridge API
```

### tavily-search.ts (Tavily Client)
```typescript
TAVILY_CONFIG.timeout = 15000;        // 15s
```

### llm-constants.ts (Central Config)
```typescript
TAVILY_CONFIG.timeout = 15000;        // 15s
```

---

## Complete Firing Order Flow Diagram

```
┌────────────────────────────────────────────────────────────────────────────────┐
│                    CLUES PROPERTY SEARCH - COMPLETE DATA FLOW                   │
│                         Total Budget: 300s (5 minutes)                          │
└────────────────────────────────────────────────────────────────────────────────┘

TIME    TIER    SOURCE                  TIMEOUT    EXECUTION MODE
═══════════════════════════════════════════════════════════════════════════════════

t=0s    ┌─────────────────────────────────────────────────────────────────────────┐
        │ TIER 1: STELLAR MLS (Bridge Interactive API)                            │
        │ Timeout: 15s │ Files: search.ts:4807, search-by-mls.ts:30              │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ • Query by MLS# or Address                                              │
        │ • Returns: 60+ fields (basics, HOA, taxes, features)                    │
        │ • Source name: "Stellar MLS"                                            │
        │ • Execution: SEQUENTIAL (must complete before Tier 2)                   │
t=15s   └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
t=15s   ┌─────────────────────────────────────────────────────────────────────────┐
        │ TIER 2: GOOGLE APIs                                                     │
        │ Timeout: 60s (shared) │ File: search.ts                                │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ Parallel Execution:                                                     │
        │ ├─ Google Geocoding → lat/lon, formatted address                       │
        │ ├─ Google Places → POI distances (grocery, hospital, park, beach)      │
        │ └─ Google Distance Matrix → commute times, transit access              │
        │ • Source names: "Google Geocode", "Google Places", "Google Distance"   │
t=75s   └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
t=75s   ┌─────────────────────────────────────────────────────────────────────────┐
        │ TIER 3: FREE APIs (Parallel)                                            │
        │ Timeout: 60s │ File: search.ts:5016-5021                               │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ All execute in parallel via Promise.all():                              │
        │ ├─ SchoolDigger      → school assignments, ratings (63, 65-73)         │
        │ ├─ FBI Crime         → crime indices (88-90)                            │
        │ ├─ WalkScore         → walk/transit/bike scores (74-80)                │
        │ ├─ FEMA NFHL         → flood zone (119-120)                            │
        │ ├─ AirNow            → air quality (117-118)                           │
        │ ├─ HowLoud           → noise level (78, 129)                           │
        │ ├─ OpenWeatherMap    → climate data (121)                              │
        │ └─ U.S. Census       → demographics                                    │
t=135s  └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
t=135s  ┌─────────────────────────────────────────────────────────────────────────┐
        │ TIER 3 (continued): TAVILY WEB SEARCH                                   │
        │ Timeout: 15s │ Files: search.ts:5117-5147, tavily-search.ts            │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ Parallel searches via Promise.all():                                    │
        │ ├─ searchAVMs(address)                                                  │
        │ │   └─ site:zillow.com → 16a_zestimate                                 │
        │ │   └─ site:redfin.com → 16b_redfin_estimate                           │
        │ ├─ searchMarketStats(city, zip)                                        │
        │ │   └─ 91_median_home_price, 92_price_per_sqft, 95_days_on_market      │
        │ ├─ searchUtilities(city, state)                                        │
        │ │   └─ 104_electric, 106_water, 109_natural_gas                        │
        │ ├─ searchPermits(address, county)                                      │
        │ │   └─ 60_permit_roof, 61_permit_hvac                                  │
        │ └─ searchPortalViews(address)                                          │
        │     └─ 169-172_views, 174_saves_favorites                              │
        │ • Source name: "Tavily", "Tavily (Zillow)", "Tavily (Redfin)"          │
t=150s  └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
═══════════════════════════════════════════════════════════════════════════════════
                    TIER 4: LLM CASCADE (Web-Search Enabled)
                    Total LLM Budget: ~150s remaining
═══════════════════════════════════════════════════════════════════════════════════
                                        │
                                        ▼
t=150s  ┌─────────────────────────────────────────────────────────────────────────┐
        │ LLM #1: PERPLEXITY (sonar-reasoning-pro)                                │
        │ Timeout: 45s │ Files: search.ts:5256, retry-llm.ts:1612                │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ Prompt Context:                                                         │
        │ "🔴 FIRING ORDER: You are the FIRST LLM in the search chain (Tier 4).  │
        │  PRIOR DATA SOURCES (Tier 3 - already ran BEFORE you):                 │
        │  - Tavily Web Search, SchoolDigger, FBI Crime, WalkScore, FEMA..."     │
        │                                                                         │
        │ Micro-prompts A-F:                                                      │
        │ ├─ A: Listing portals, AVMs, neighborhood pricing                      │
        │ ├─ B: County records, permits, legal                                   │
        │ ├─ C: Schools, walkability, crime                                      │
        │ ├─ D: Utilities, connectivity, structure ages                          │
        │ ├─ E: Comparable sales, financing                                      │
        │ └─ F: EV charging, smart home, views/saves                             │
        │ • Source name: "Perplexity"                                            │
t=195s  └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
t=195s  ┌─────────────────────────────────────────────────────────────────────────┐
        │ LLM #2: GEMINI (gemini-3-pro-preview + Google Search)                   │
        │ Timeout: 180s │ Files: search.ts:5286, gemini-prompts.ts               │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ Prompt Context:                                                         │
        │ "🟡 FIRING ORDER: You are the 3rd data source in the search chain.     │
        │  TIER 3 DATA SOURCES (already ran BEFORE you):                         │
        │  - Tavily Web Search, Free APIs..."                                    │
        │                                                                         │
        │ • Uses Google Search grounding tool                                     │
        │ • 47 high-velocity fields                                              │
        │ • Source name: "Gemini"                                                │
t=375s  └─────────────────────────────────────────────────────────────────────────┘
                                        │
                            (Cascade continues if time permits)
                                        │
                                        ▼
        ┌─────────────────────────────────────────────────────────────────────────┐
        │ LLM #3: GPT-4o (+ web_search tool)                                      │
        │ Timeout: 180s │ File: search.ts                                        │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ "🟠 FIRING ORDER: 3rd LLM in chain (after Perplexity → Gemini).        │
        │  PRIOR: Tier 3 + Perplexity, Gemini"                                   │
        │ • Uses OpenAI Responses API with web_search                            │
        │ • Source name: "GPT" or "GPT-4o"                                       │
        └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
        ┌─────────────────────────────────────────────────────────────────────────┐
        │ LLM #4: CLAUDE SONNET (claude-sonnet-4-5 + web_search beta)             │
        │ Timeout: 180s │ File: search.ts                                        │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ "🔵 FIRING ORDER: 4th LLM in chain. Grok and Opus fire AFTER.          │
        │  PRIOR: Tier 3 + Perplexity, Gemini, GPT"                              │
        │ • Uses Anthropic web_search beta tool                                   │
        │ • Source name: "Claude Sonnet"                                         │
        └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
        ┌─────────────────────────────────────────────────────────────────────────┐
        │ LLM #5: GROK (grok-4-1-fast)                                            │
        │ Timeout: 180s │ File: search.ts                                        │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ "🟣 FIRING ORDER: 5th LLM in chain.                                    │
        │  PRIOR: Tier 3 + Perplexity, Gemini, GPT, Claude Sonnet"               │
        │ • Uses Tavily as web search backend for tool calls                     │
        │ • Source name: "Grok"                                                  │
        └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
═══════════════════════════════════════════════════════════════════════════════════
                    TIER 5: CLAUDE OPUS (No Web Search)
═══════════════════════════════════════════════════════════════════════════════════
                                        │
                                        ▼
        ┌─────────────────────────────────────────────────────────────────────────┐
        │ LLM #6: CLAUDE OPUS (claude-opus-4-5 - NO WEB ACCESS)                   │
        │ Timeout: 180s │ File: search.ts                                        │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ "⚫ FIRING ORDER: 6th and FINAL LLM in chain.                          │
        │  PRIOR: Tier 3 + ALL 5 Tier 4 LLMs"                                    │
        │                                                                         │
        │ • NO web search capability                                              │
        │ • Uses training knowledge only                                          │
        │ • Fires LAST as final fallback                                         │
        │ • Forbidden from guessing live market data                             │
        │ • Source name: "Claude Opus"                                           │
        └─────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
        ┌─────────────────────────────────────────────────────────────────────────┐
        │ FINAL OUTPUT: arbitrationPipeline.getResult()                          │
        ├─────────────────────────────────────────────────────────────────────────┤
        │ {                                                                       │
        │   fields: { [key]: { value, source, confidence } },                    │
        │   source_breakdown: {                                                  │
        │     "Stellar MLS": 45,                                                 │
        │     "Google Places": 8,                                                │
        │     "Tavily": 6,                                                       │
        │     "Perplexity": 12,                                                  │
        │     "Gemini": 8,                                                       │
        │     ...                                                                │
        │   },                                                                   │
        │   total_fields_found: 138                                              │
        │ }                                                                       │
        └─────────────────────────────────────────────────────────────────────────┘
```

---

## File Cross-Reference Matrix

| Source | search.ts | retry-llm.ts | search-by-mls.ts | search-stream.ts | tavily-search.ts | llm-constants.ts | source-constants.ts | perplexity-prompts.ts | gemini-prompts.ts |
|--------|-----------|--------------|------------------|------------------|------------------|------------------|---------------------|----------------------|-------------------|
| Stellar MLS | ✓ (15s) | - | ✓ (15s) | ✓ | - | - | ✓ | - | - |
| Google Geocode | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| Google Places | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| Google Distance | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| SchoolDigger | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| FBI Crime | ✓ (60s) | - | - | ✓ | - | - | ✓ | - | - |
| WalkScore | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| FEMA | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| AirNow | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| HowLoud | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| Weather | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| Census | ✓ (60s) | - | - | ✓ | - | - | - | - | - |
| **Tavily** | **✓ (15s)** | **✓** | - | **✓** | **✓ (15s)** | **✓ (15s)** | **✓** | **✓** | **✓** |
| Perplexity | ✓ (45s) | ✓ (45s) | - | ✓ | - | - | - | ✓ | - |
| Gemini | ✓ (180s) | ✓ (180s) | - | ✓ | - | - | - | - | ✓ |
| GPT | ✓ (180s) | ✓ (180s) | - | ✓ | - | - | - | - | - |
| Claude Sonnet | ✓ (180s) | ✓ (180s) | - | ✓ | - | - | - | - | - |
| Grok | ✓ (180s) | ✓ (180s) | - | ✓ | - | - | - | - | - |
| Claude Opus | ✓ (180s) | ✓ (180s) | - | ✓ | - | - | - | - | - |

---

## Timeout Summary by Tier

| Tier | Sources | Individual Timeout | Execution Mode | Max Tier Time |
|------|---------|-------------------|----------------|---------------|
| 1 | Stellar MLS | 15s | Sequential | 15s |
| 2 | Google APIs (3) | 60s each | Parallel | 60s |
| 3 | Free APIs (8) | 60s each | Parallel | 60s |
| 3 | Tavily | 15s | Sequential (after Free) | 15s |
| 4 | Perplexity | 45s | Sequential | 45s |
| 4 | Gemini | 180s | Sequential | 180s |
| 4 | GPT | 180s | Sequential | 180s |
| 4 | Claude Sonnet | 180s | Sequential | 180s |
| 4 | Grok | 180s | Sequential | 180s |
| 5 | Claude Opus | 180s | Sequential | 180s |

**Note:** In practice, LLMs typically respond in 20-60s. The cascade exits early when sufficient fields are found.

---

## LLM Prompt Firing Order Context

Each LLM prompt now includes explicit firing order awareness:

```
┌─────────────────────────────────────────────────────────────────┐
│ PERPLEXITY (LLM #1):                                            │
│ "🔴 FIRING ORDER: You are the FIRST LLM (Tier 4).              │
│  PRIOR: Tier 3 (Tavily, Free APIs)"                            │
├─────────────────────────────────────────────────────────────────┤
│ GEMINI (LLM #2):                                                │
│ "🟡 FIRING ORDER: You are the 3rd data source.                 │
│  PRIOR: Tier 3 (Tavily, Free APIs)"                            │
├─────────────────────────────────────────────────────────────────┤
│ GPT (LLM #3):                                                   │
│ "🟠 FIRING ORDER: 3rd LLM (after Perplexity → Gemini).         │
│  PRIOR: Tier 3 + Perplexity, Gemini"                           │
├─────────────────────────────────────────────────────────────────┤
│ CLAUDE SONNET (LLM #4):                                         │
│ "🔵 FIRING ORDER: 4th LLM. Grok and Opus fire AFTER.           │
│  PRIOR: Tier 3 + Perplexity, Gemini, GPT"                      │
├─────────────────────────────────────────────────────────────────┤
│ GROK (LLM #5):                                                  │
│ "🟣 FIRING ORDER: 5th LLM.                                     │
│  PRIOR: Tier 3 + Perplexity, Gemini, GPT, Sonnet"              │
├─────────────────────────────────────────────────────────────────┤
│ CLAUDE OPUS (LLM #6):                                           │
│ "⚫ FIRING ORDER: 6th and FINAL. NO web search.                │
│  PRIOR: Tier 3 + ALL 5 Tier 4 LLMs"                            │
└─────────────────────────────────────────────────────────────────┘
```

---

## Updated: 2026-01-08

Files involved in Tavily integration:
1. api/property/tavily-search.ts (NEW)
2. api/property/search.ts
3. api/property/retry-llm.ts
4. api/property/search-by-mls.ts
5. api/property/search-stream.ts
6. api/property/source-constants.ts
7. api/property/llm-constants.ts
8. api/property/perplexity-prompts.ts
9. src/config/gemini-prompts.ts
10. src/lib/field-normalizer.ts
