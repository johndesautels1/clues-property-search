...
function scoreToBandColor(score) {
...
function scoreLowerIsBetter(values) {
...
function scoreHigherIsBetter(values) {
  if (!values.length) return [];
  const min = Math.min(...values);
  const max = Math.max(...values);
  if (max === min) return values.map(() => 50);
...
function SmartScoreBadge({ value }) {
...
function PriceSpectrumChart({ homes }) {
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 40 }}>
            ...
            />
            <Tooltip
              formatter={(value) => [formatCurrency(value), 'Listing price']}
...
function PricePerSqFtChart({ homes }) {
  const data = homes.map((h, index) => ({
    name: h.name,
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            ...
            />
            <Tooltip
              formatter={(value) => [`$${value.toLocaleString()}/sqft`, 'Price per Sq Ft']}
              wrapperStyle={tooltipWrapperStyle}
              contentStyle={tooltipContentStyle}
            ...
            <Bar dataKey="pricePerSqFt" radius={[0, 4, 4, 0]}>
              <LabelList
                ...
                position="right"
                formatter={(value) => `$${value.toLocaleString()}/sqft`}
                style={{ fill: COLORS.text, fontSize: 10, fontWeight: 700 }}
              />
...
function ListVsMarketChart({ homes }) {
  // Calculate how much list price is over/under market value
  const getListingColor = (listPrice, marketValue) => {
...
function EquityScoreChart({ homes }) {
  const data = homes.map((h, index) => {
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            ...
            />
            <Tooltip
              formatter={(value) => [`${value.toFixed(0)}/100`, 'Value score']}
              wrapperStyle={tooltipWrapperStyle}
              contentStyle={tooltipContentStyle}
              labelStyle={tooltipLabelStyle}
              itemStyle={tooltipItemStyle}
...
function PriceComponentsChart({ homes }) {
  return (
...
function RadarChartComponent({ homes }) {
  // Base metrics
...
function EquityGaugeChart({ homes }) {
  // List vs Market: lower (more under market) is better
  const listVsMarketPercents = homes.map(
    (h) => ((h.listingPrice - h.marketValue) / h.marketValue) * 100,
  );
...
function AppreciationChart({ homes }) {
  const data = homes.map((h, index) => {
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
            ...
            />
            <Tooltip
              formatter={(value) => [formatPercent(value), 'Appreciation since last sale']}
              wrapperStyle={tooltipWrapperStyle}
              contentStyle={tooltipContentStyle}
              labelStyle={tooltipLabelStyle}
              itemStyle={tooltipItemStyle}
...
function OverallScoreChart({ homes }) {
  const listingPrices = homes.map((h) => h.listingPrice);
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            ...
            />
            <Tooltip
              formatter={(value) => [`${value.toFixed(0)}/100`, 'Overall score']}
              wrapperStyle={tooltipWrapperStyle}
              contentStyle={tooltipContentStyle}
              labelStyle={tooltipLabelStyle}
              itemStyle={tooltipItemStyle}
...
function ValuePyramidChart({ homes }) {
...
export default function RealEstateDashboard({ homes = sampleHomes }) {
