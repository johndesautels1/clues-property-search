...
export function extractPropertyData(property: Property): OliviaEnhancedPropertyInput {
  ...
  const getValue = <T,>(field: any): T | null | undefined => {
...
const OLIVIA_SYSTEM_PROMPT = `You are Olivia, a world-class real estate AI advisor for CLUES™.

CRITICAL: Analyze ALL 168 property fields across 22 categories to provide comprehensive analysis.

Return ONLY valid JSON following OliviaEnhancedAnalysisResult interface.
...
export async function analyzeWithOliviaEnhanced(
...
): Promise<OliviaEnhancedAnalysisResult> {
  ...
  // Format ALL 168 fields for all properties
  const propertyDetails = request.properties
    .map((p, i) => formatPropertyForPrompt(p, i))
    .join('\n\n');

  const userPrompt = `Analyze these ${request.properties.length} properties using ALL 168 fields across 22 categories.

${propertyDetails}

Buyer Profile: ${request.buyerProfile || 'Not specified'}

GENERATE COMPREHENSIVE ANALYSIS with Investment Grade, Section Analysis, Rankings, and Recommendations.
...
Return ONLY valid JSON. No markdown.`;
  ...
  try {
    ...
    const result = JSON.parse(cleanText) as OliviaEnhancedAnalysisResult;
