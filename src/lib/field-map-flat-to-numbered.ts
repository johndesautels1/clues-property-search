/**
 * CLUES Property Dashboard - Flat to Numbered Field Mapping
 * 
 * SHARED MODULE - Used by:
 * - api/property/search-stream.ts
 * - api/property/retry-llm.ts
 * 
 * Maps LLM flat field names (e.g., "bedrooms") to numbered schema keys (e.g., "17_bedrooms")
 * This ensures consistent field mapping across all LLM response handlers.
 * 
 * UPDATED: 2025-12-01 - Extracted from duplicated code per 5-Agent Deep Audit
 */

/**
 * Map of flat LLM field names to numbered schema keys
 * All field numbers align with fields-schema.ts (SOURCE OF TRUTH)
 */
export const FLAT_TO_NUMBERED_FIELD_MAP: Record<string, string> = {
  // Address & Identity (1-9)
  'full_address': '1_full_address',
  'address': '1_full_address',
  'mls_primary': '2_mls_primary',
  'mls_number': '2_mls_primary',
  'new_construction_yn': '3_new_construction_yn',
  'new_construction': '3_new_construction_yn',
  ...
  '16a_zestimate': '16a_zestimate',
  'redfin_estimate': '16b_redfin_estimate',
  '16b_redfin_estimate': '16b_redfin_estimate',
  'first_american_avm': '16c_first_american_avm',
  '16c_first_american_avm': '16c_first_american_avm',
  'quantarium_avm': '16d_quantarium_avm',
  '16d_quantarium_avm': '16d_quantarium_avm',
  'ice_avm': '16e_ice_avm',
  '16e_ice_avm': '16e_ice_avm',
  'collateral_analytics_avm': '16f_collateral_analytics_avm',
  '16f_collateral_analytics_avm': '16f_collateral_analytics_avm',
  'rent_zestimate': '181_rent_zestimate',
  '181_rent_zestimate': '181_rent_zestimate',
  ...
  'hoa_fee_monthly': '31_hoa_fee_annual', // Requires monthly→annual conversion
  ...
  'elementary_distance_mi': '67_elementary_distance_mi',
  ...
  'middle_distance_mi': '70_middle_distance_mi',
  ...
  'high_distance_mi': '73_high_distance_mi',
  ...
  'carport_yn': '139_carport_yn',
  'carport': '139_carport_yn',
  'carport_spaces': '140_carport_spaces',
  'garage_attached_yn': '141_garage_attached_yn',
  'garage_attached': '141_garage_attached_yn',
  'parking_features': '142_parking_features',
  'assigned_parking_spaces': '143_assigned_parking_spaces',

  // Stellar MLS Building (144-148)
  'floor_number': '144_floor_number',
  'building_total_floors': '145_building_total_floors',
  'total_floors': '145_building_total_floors',
  'building_name_number': '146_building_name_number',
  'building_elevator_yn': '147_building_elevator_yn',
  'elevator': '147_building_elevator_yn',
  'floors_in_unit': '148_floors_in_unit',

  // Stellar MLS Legal (149-154)
  'subdivision_name': '149_subdivision_name',
  'subdivision': '149_subdivision_name',
  'legal_description': '150_legal_description',
  'homestead_yn': '151_homestead_yn',
  'homestead': '151_homestead_yn',
  'cdd_yn': '152_cdd_yn',
  'cdd': '152_cdd_yn',
  'annual_cdd_fee': '153_annual_cdd_fee',
  'cdd_fee': '153_annual_cdd_fee',
  ...
  'can_be_leased': '160_can_be_leased_yn',
  'minimum_lease_period': '161_minimum_lease_period',
  'lease_restrictions_yn': '162_lease_restrictions_yn',
  'lease_restrictions': '162_lease_restrictions_yn',
  'pet_size_limit': '163_pet_size_limit',
  'max_pet_weight': '164_max_pet_weight',
  'association_approval_yn': '165_association_approval_yn',
  'association_approval': '165_association_approval_yn',

  // Stellar MLS Features (166-168)
  'community_features': '166_community_features',
  'interior_features': '167_interior_features',
  'exterior_features': '168_exterior_features',
 ...
 * Handles both direct mapping and case-insensitive lookups
 * 
 * @param fields - Raw fields from LLM response
 * @param source - Source name for logging
 * @returns Fields with numbered keys
 */
export function mapFlatFieldsToNumbered(
...
/**
 * Check if a key requires HOA monthly→annual conversion
 */
export function isMonthlyHoaFeeKey(key: string): boolean {
  const lowerKey = key.toLowerCase();
  return lowerKey === 'hoa_fee_monthly' || 
         lowerKey === 'monthly_hoa_fee' || 
         lowerKey === 'hoa_monthly';
}
...
export function convertMonthlyHoaToAnnual(monthlyValue: string | number | null | undefined): number | null {
  ...
  // Parse the value
  let monthly: number;
  if (typeof monthlyValue === 'number') {
    monthly = monthlyValue;
  } else if (typeof monthlyValue === 'string') {
    // Remove currency symbols and commas
    const cleaned = monthlyValue.replace(/[$,]/g, '').trim();
    monthly = parseFloat(cleaned);
  } else {
    return null;
  }
