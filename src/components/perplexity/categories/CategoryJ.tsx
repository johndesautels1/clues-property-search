...
function LocationExcellenceSpider({ properties }: CategoryJProps) {
  ...
  const propertyData = comparisonProperties.map((p, idx) => {
    ...
    return {
      ...
      grocery: getVal(p.location?.distanceGroceryMiles) ? 100 - Math.min(getVal(p.location?.distanceGroceryMiles)! * 10, 100) : 0,
      hospital: getVal(p.location?.distanceHospitalMiles) ? 100 - Math.min(getVal(p.location?.distanceHospitalMiles)! * 5, 100) : 0,
      park: getVal(p.location?.distanceParkMiles) ? 100 - Math.min(getVal(p.location?.distanceParkMiles)! * 20, 100) : 0,
      beach: getVal(p.location?.distanceBeachMiles) ? 100 - Math.min(getVal(p.location?.distanceBeachMiles)! * 5, 100) : 0,
      color: propColor,
      propertyNum: idx + 1,
    };
  });
