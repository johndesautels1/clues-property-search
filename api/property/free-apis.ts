...
import { calculateCoastlineDistance, validateBeachDistance, getSeaLevelRiskFromDistance, FLORIDA_COASTLINE_POINTS } from './coastline-utils.js';
    ...
    for (const place of placeTypes) {
      try {
        ...
        if (distResult.success && distResult.data?.rows?.[0]?.elements?.[0]?.distance) {
          ...
          const googleMiles = parseFloat((meters / 1609.34).toFixed(1));

          // BEACH VALIDATION: Compare Google result against coastline calculation
          // Google may find pools, beach clubs, restaurants instead of actual ocean
          if (place.type === 'beach') {
            const validation = validateBeachDistance(googleMiles, lat, lon);
            fields[place.field] = {
            ...
            if (validation.googleRejected) {
              console.warn(`⚠️ [FREE-APIS] Beach validation REJECTED Google: ${googleMiles} mi to "${nearest.name}"`);
              console.warn(`⚠️ [FREE-APIS] Using coastline distance: ${validation.distance.toFixed(1)} mi`);
            }
          } else {
            setField(fields, place.field, googleMiles, 'Google Places');
          }
        }
      } catch (e) {
...
export async function callCrimeGrade(lat: number, lon: number, address: string): Promise<ApiResult> {
  ...
  try {
    ...
    const [violentResult, propertyResult] = await Promise.all([
      safeFetch<any>(violentUrl, undefined, 'FBI-Crime-Violent', 30000),
      safeFetch<any>(propertyUrl, undefined, 'FBI-Crime-Property', 30000)
...
export async function callNOAAClimate(lat: number, lon: number, zip: string, county: string): Promise<ApiResult> {
  ...
  try {
    ...
    const fetchResult = await safeFetch<any[]>(url, undefined, 'NOAA-Climate', 30000); // 30s timeout - REDUCED from 60s
...
export async function callNOAAStormEvents(county: string, state: string = 'FL'): Promise<ApiResult> {
  ...
  try {
    ...
    const fetchResult = await safeFetch<any[]>(url, undefined, 'NOAA-Storm', 30000); // 30s timeout - REDUCED from 60s
...
export async function callNOAASeaLevel(lat: number, lon: number, beachDistanceMiles?: number): Promise<ApiResult> {
  ...
  try {
    // Calculate distance to coast using shared utilities from coastline-utils.ts
    // CRITICAL: Validate Google Places beach distance against coastline calculation
    // Google may find pools, beach clubs, restaurants instead of actual ocean
    ...
    if (beachDistanceMiles !== undefined && beachDistanceMiles !== null && !isNaN(beachDistanceMiles)) {
      // Validate Google Places distance against coastline calculation
      const validation = validateBeachDistance(beachDistanceMiles, lat, lon);
      distanceToCoast = validation.distance;
      distanceSource = validation.source;

      if (validation.googleRejected) {
        console.warn(`[NOAA Sea Level] ⚠️ Google beach distance REJECTED: ${beachDistanceMiles.toFixed(1)} mi`);
        console.warn(`[NOAA Sea Level] Using coastline calculation: ${distanceToCoast.toFixed(1)} mi`);
        console.warn(`[NOAA Sea Level] Reason: ${validation.reason}`);
      } else {
        console.log(`[NOAA Sea Level] Using validated beach distance: ${distanceToCoast.toFixed(1)} mi`);
      }
    } else {
      // Fallback: Use shared coastline calculation from coastline-utils.ts
      distanceToCoast = calculateCoastlineDistance(lat, lon);
      distanceSource = 'Coastline Calculation (Estimated)';
      console.warn(`[NOAA Sea Level] ⚠️ Using coastline calculation (${distanceToCoast.toFixed(1)} mi) - Google unavailable`);
    }

    // Use shared sea level risk calculation from coastline-utils.ts
    const riskInfo = getSeaLevelRiskFromDistance(distanceToCoast);
...
export async function callUSGSElevation(lat: number, lon: number): Promise<ApiResult> {
  ...
  try {
    ...
    const fetchResult = await safeFetch<any>(url, undefined, 'USGS-Elevation', 30000); // 30s timeout - REDUCED from 60s
...
export async function callUSGSEarthquake(lat: number, lon: number): Promise<ApiResult> {
  ...
  try {
    ...
    const fetchResult = await safeFetch<any>(url, undefined, 'USGS-Earthquake', 30000); // 30s timeout - REDUCED from 60s
...
export async function callEPAFRS(lat: number, lon: number): Promise<ApiResult> {
  ...
  try {
    ...
    const fetchResult = await safeFetch<any>(url, undefined, 'EPA-FRS', 30000); // 30s timeout - REDUCED from 60s
