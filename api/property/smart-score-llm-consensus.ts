 ...
 */
async function callGPT4o(prompt: string): Promise<LLMResponse> {
...
async function calculateConsensus(
  ...
  try {
    tiebreakerResult = await callGPT4o(prompt);
    tiebreakerModel = 'gpt-4o';
    console.log('[LLM Consensus] Tiebreaker: GPT-4o');
  } catch (error) {
    console.log('[LLM Consensus] GPT-4o failed, trying Grok...');
