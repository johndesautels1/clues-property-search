...
const TAVILY_TIMEOUT = 45000; // 45 seconds for Tavily web searches (Tier 3) - INCREASED from 30s on 2026-01-13 for fields 169-174
...
const FIELD_TYPE_MAP: Record<string, FieldType> = {
  ...
  '113_fiber_available': 'boolean', 'fiber_available': 'boolean',
  ...
  '127_superfund_site_nearby': 'boolean', 'superfund_site_nearby': 'boolean',
  ...
  // ================================================================
  // GROUP 23 (W): Market Performance (Fields 169-181) - Updated 2026-01-11
  // ================================================================
...
function normalizeUtilityBillToMonthly(fieldKey: string, value: any): { value: number; wasNormalized: boolean } | null {
  ...
  if (fieldKey === '107_avg_water_bill' || fieldKey === 'avg_water_bill') {
    if (numericValue > 120) {
      ...
      console.log(`🔄 WATER BILL NORMALIZED: $${numericValue} bi-monthly → $${monthly}/month`);
...
async function getDistances(lat: number, lon: number): Promise<Record<string, any>> {
  ...
  // Use shared coastline calculation from coastline-utils.ts (Single Source of Truth)
    ...
    try {
      ...
      if (searchData.results && searchData.results.length > 0) {
        ...
        if (distData.rows?.[0]?.elements?.[0]?.distance) {
          ...
          // but may also find pools, beach clubs, restaurants - validate against coastline
          if (place.type === 'beach') {
            // Use shared validation from coastline-utils.ts
            // This compares Google result vs actual coastline distance
            // If difference > 3 miles, Google is probably wrong (found pool/club/restaurant)
            const validation = validateBeachDistance(parseFloat(googleMiles), lat, lon);
            fields[place.field] = {
              value: parseFloat(validation.distance.toFixed(1)),
              source: validation.source,
              confidence: validation.googleRejected ? 'Medium' : 'High',
              details: validation.googleRejected
                ? `Google found "${nearest.name}" at ${googleMiles} mi - REJECTED (coastline: ${actualCoastDistance.toFixed(1)} mi)`
                : nearest.name
            };
            if (validation.googleRejected) {
              console.warn(`⚠️ [BEACH VALIDATION] Rejected Google result: "${nearest.name}" at ${googleMiles} mi`);
              console.warn(`⚠️ [BEACH VALIDATION] Using coastline distance: ${validation.distance.toFixed(1)} mi`);
              console.warn(`⚠️ [BEACH VALIDATION] Reason: ${validation.reason}`);
            }
          } else {
...
169. months_of_inventory, 170. new_listings_30d, 171. homes_sold_30d, 172. median_dom_zip,
173. price_reduced_percent, 174. homes_under_contract, 175. market_type, 176. avg_sale_to_list_percent,
177. avg_days_to_pending, 178. multiple_offers_likelihood, 179. appreciation_percent, 180. price_trend, 181. rent_zestimate
`;
...
PRIOR DATA SOURCES (already ran BEFORE you):
- Tier 3: Tavily Web Search, SchoolDigger, FBI Crime, WalkScore, FEMA, AirNow, Census, Weather
- Tier 4 LLMs: Perplexity (Gemini available on-demand only)
...
8) Market Performance:
   ...
   - "site:zillow.com [ADDRESS] rent zestimate" → 181_rent_zestimate
...
🔵 FIRING ORDER: You are the 3rd LLM in the search chain (after Perplexity → GPT). Grok and Opus fire AFTER you.
PRIOR DATA SOURCES (already ran BEFORE you):
- Tier 3: Tavily Web Search, SchoolDigger, FBI Crime, WalkScore, FEMA, AirNow, Census, Weather
- Tier 4 LLMs: Perplexity, GPT (Gemini available on-demand only)
You ONLY search for fields that prior sources did NOT find.
...
- 175_market_type: Buyer's/Balanced/Seller's Market
- 176_avg_sale_to_list_percent: Sale-to-list price ratio
- 177_avg_days_to_pending: Days from listing to pending
- 178_multiple_offers_likelihood: Multiple offers likelihood
- 179_appreciation_percent: Year-over-year appreciation
- 180_price_trend: Price trend (Rising/Stable/Falling)
- 181_rent_zestimate: Zillow rent estimate
...
async function callGemini(address: string): Promise<any> {
  ...
  if (!response) {
    ...
    return { error: `All Gemini models failed: ${lastError}`, fields: {}, llm: 'Gemini' };
  }

  try {
    const elapsed = Date.now() - startTime;
    console.log(`⏱️ [Gemini] Model: ${usedModel} | Response time: ${elapsed}ms ${elapsed < 2000 ? '⚠️ TOO FAST' : '✅ Searched'}`);
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const propertyData: PropertyData = {
      ...
      field_169_months_of_inventory: getFieldValue('169_months_of_inventory'),
      field_170_new_listings_30d: getFieldValue('170_new_listings_30d'),
      field_171_homes_sold_30d: getFieldValue('171_homes_sold_30d'),
      field_172_median_dom_zip: getFieldValue('172_median_dom_zip'),
      field_173_price_reduced_percent: getFieldValue('173_price_reduced_percent'),
      field_174_homes_under_contract: getFieldValue('174_homes_under_contract'),
