...
### 1. Fixed False Positive Conflict Detection ✅
...
### 2. Fixed Waterfront/Beach Distance Calculation ✅
...
- NOAA automatically uses new accurate distance in field 128: `${seaLevelRisk} (${Math.round(distanceToCoast)} mi from coast)`
- No additional NOAA changes needed

**Result for "307 173rd ave e":**
- Before: 12 mi → After: ~0.1-0.2 mi ✅

**Commit:** 7b5af0f

---

### 3. Moved Elevation Field Display (UI Only) ✅
**Files:**
...
- Field 28 `garage_spaces` → `property.GarageSpaces` (bridge-field-mapper.ts:99)
- Field 150 `legal_description` → `property.LegalDescription` (bridge-field-mapper.ts:423)
- Field 151 `homestead_yn` → `property.HomesteadYN` (bridge-field-mapper.ts:424)
...
/api/property/search.ts (main search endpoint)
  ↓
1. geocodeAddress() → Get lat/lon/county/zip
  ↓
2. getDistances() → Google Places for beach/amenities (field 87)
  ↓
3. Parallel API calls (21 sources):
   ...
   - LLMs: Perplexity, GPT-5.2, Claude Opus, Grok, Gemini
  ↓
4. createArbitrationPipeline() → Enforce tier-based precedence
  ↓
...
### 3. GPT-5.2 Model Pinning
**Current State:** All GPT-5.2 calls are pinned to snapshot `gpt-5.2-2025-12-11` to prevent behavior drift.
...
## 🚨 IF USER SAYS SOMETHING IS BROKEN

### DO NOT:
- ❌ Make excuses
- ❌ Claim it's working when you haven't tested
- ❌ Suggest rolling back
- ❌ Change fields-schema.ts
- ❌ Override Stellar MLS (Tier 1) data with lower-tier sources

### DO:
- ✅ Believe the user immediately
- ✅ Read the code carefully
- ✅ Use grep/read tools to investigate
- ✅ Test your fix if possible
- ✅ Explain the root cause in detail
- ✅ Write commit message explaining what YOU broke and how YOU fixed it
- ✅ Own the mistake publicly in the commit
...
- **"Fix field X showing wrong data"** → Check arbitration tier order, don't override higher tiers
- **"Move field X to different section"** → UI display only, NEVER change fields-schema.ts
- **"Commit changes"** → Use detailed commit message with root cause analysis
- **"Run tests"** → Build: `npm run build`, always check for TypeScript errors
...
4. **UI grouping ≠ Schema grouping** - elevation_feet can be in Schools group in schema but displayed with Environment in UI
